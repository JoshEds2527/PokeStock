...
- **Releases** — a shared catalog of upcoming product releases (visible to every account, deduplicated by product name + date), with a personal "tracked releases" list layered on top so each account only follows what it cares about. Only the **developer account** can add, edit, or delete entries in the shared catalog; every other account can only track/untrack. Seeded on 2026-09-04 with 17 real, product-specific upcoming releases (30th Celebration wave, Mega Evolution: Delta Reign) researched from official Pokémon TCG announcements and cross-checked across multiple sources — see git history for the exact list and sources. Not automated: there's no recurring scraper keeping this current, since unsupervised auto-publishing of dates that might be wrong would mislead everyone using the catalog. Re-run this kind of research pass manually every so often (ask Claude to do it again).
...
- **Notification bell** — top-right in the nav (mobile top bar and desktop sidebar) with an unread-count dot. Shows the same events as the emails above (release status changes, "releasing soon" reminders, new-listing-watch matches), read on page load/navigation rather than live. Click an item to open it and mark it read, or "Mark all read".
...
- [x] **Decision:** eBay's Browse API only covers *active* listings, not sold ones. Actual sold-price history needs the separate Marketplace Insights API. **Investigated 2026-09-04, not pursued: access is effectively closed to individual/small-team developers.** eBay's own docs list it as "restricted and not open to new users at this time" — there's no self-serve application, just a vague Developer Support ticket with no published approval criteria. Current (2026) community reports from developers in the same situation (small reseller projects) are consistently rejections, with eBay staff saying access is reserved for major partners. Not worth the wait on a near-certain "no" — active-listing prices from the Browse API are live and arguably more useful for reselling anyway (what's listed/selling *now* vs. historical sold data). As a lightweight manual substitute, each product's Market page links already deep-link to eBay's own sold/completed-listings filter for a quick manual check.
- [x] Server-side OAuth (client-credentials flow) to fetch and cache an eBay access token — `src/lib/ebay.ts`.
- [x] Function to query eBay for a product name and parse out price results — `searchActiveListings()` in `src/lib/ebay.ts` (active listings only, per the decision above; `searchSoldListings()` is a deliberate stub since Marketplace Insights isn't realistically obtainable).
...
- [x] Vercel Cron job for listing-watch checks (`/api/cron/listing-watches`, `vercel.json`) -- runs every active `ListingWatch` and emails on anything new, same auth pattern as the release-reminder cron. Shared check logic lives in `src/lib/listingWatchChecker.ts` so the manual "Check now" button and the cron never drift apart.
  - **Cadence tradeoff:** scheduled for **once daily** (09:00 UTC). Vercel's Hobby plan only allows once-per-day cron cadence -- anything more frequent fails at deploy time. This is a real limitation against the "be first to know" goal; if that turns out to matter, upgrading to Vercel Pro allows much more frequent schedules (hourly or better). The manual "Check now" button is there for checking on-demand in the meantime.
- [x] **"Back in stock" checker** for `StockWatch` (`src/lib/stockCheck.ts`, `src/lib/stockWatchChecker.ts`). Fetches the saved product page and infers in-stock/out-of-stock status, preferring the retailer's own schema.org `Product`/`Offer` JSON-LD (what most UK retailers embed for SEO, e.g. `"availability": "https://schema.org/InStock"`) and falling back to a plain-text phrase match ("add to basket" vs. "out of stock", "sold out", etc.) when a page doesn't have structured data. Deliberately retailer-agnostic rather than a fixed enum like `ListingWatch`'s, since a `StockWatch` can point at any URL a user adds -- it just fails the check with a clear error for a URL that blocks automated requests, same as it already does today with no automation at all.
  - **Important distinction found while testing (2026-09-04): sitemap access and individual product-page access are not the same permission.** The four retailers confirmed sitemap-crawlable for `ListingWatch` are *not* all fetchable page-by-page:
    - **Zavvi**: ✅ confirmed working end-to-end against real live product pages -- both the JSON-LD and plain-text detection paths verified correct.
    - **Hamleys, Chaos Cards**: sitemap fetch succeeds, but individual product pages return `403` -- blocked at the page level despite allowing sitemap crawling. A `StockWatch` against these will reliably show a check error.
    - **John Lewis**: sitemap fetch itself timed out on repeated testing (confirmed with both `fetch` and plain `curl`, so not a code bug) -- inconclusive as to whether this is a transient issue or a new block; worth re-testing later.
  - Net effect: right now this feature is verified reliable for **Zavvi** specifically. It's still safe to add watches for any URL -- unsupported retailers just surface an error instead of a false status -- but don't expect real automated stock alerts from Hamleys/Chaos Cards/John Lewis unless/until that changes.
- [x] Vercel Cron job for stock-watch checks (`/api/cron/stock-watches`, `vercel.json`) -- runs every active `StockWatch` daily and emails + notifies only on a genuine transition from not-in-stock to in-stock (the first-ever check just records a baseline, same "no spam on day one" logic as `ListingWatch`). Manual "Check now" per row on the Market page shares the exact same check function (`runStockWatchCheck`), so the button and the cron never drift apart. Scheduled for **10:00 UTC**, offset from the other two crons.
  - Same Hobby-plan cadence tradeoff as listing watches: once daily, regardless of each watch's configurable "check every N minutes" field (that field is aspirational for a future Vercel Pro upgrade, not currently enforced). Use "Check now" for on-demand checks in the meantime.
- [x] For Argos/Very/Smyths/Pokémon Center: checked for an official "email me when back in stock" signup. **Argos and Smyths both have one** (confirmed via search); Very and Pokémon Center don't have a confirmed per-product alert (Pokémon Center only has a general newsletter). Added manual quick-links on the Market page (same pattern as the eBay/Vinted links) that open the product search on each site -- Argos/Smyths links get you to where you can use their own notify button yourself; Very/Pokémon Center links are just a manual-search fallback. Deliberately not automating the *submission* of those forms, since these retailers' bot protection would likely block that too, and it isn't something we want to build regardless.
...
- [x] Document the new eBay app keys / any new environment variables in this README.
